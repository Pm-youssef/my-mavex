            ...
            <nav className="order-2 hidden md:flex flex-1 items-center justify-center space-x-6 space-x-reverse h-16">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative px-3 py-2 text-base font-semibold transition-all duration-300 ease-smooth group hover:translate-y-[1px] ${
                    isScrolled ? 'text-[#0c1420]' : 'text-white'
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 right-0 h-0.5 bg-yellow-500 transition-all duration-300 ease-smooth ${
